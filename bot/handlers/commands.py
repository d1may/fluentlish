...
from bot.services import parser_cambridge, formatting, word_selector
from bot.database import queries

router = Router()

RESOURCES_DIR = Path(__file__).resolve().parent.parent / "resources"
SONG = types.input_file.FSInputFile(str(RESOURCES_DIR / "knock.ogg"))
ABRAMS = types.input_file.FSInputFile(str(RESOURCES_DIR / "abrams.jpg"))
...
async def command_start_handler(message: types.Message):
    logging.info("New user: chat_id=%s", message.chat.id)
    user_id = message.from_user.id

    await message.answer(
        f"🇺🇸 knock knock...\n"
        f"Hi {message.from_user.first_name}, I'm your English learning bot. "
        f"I will send you one English word every hour, I will try to match "
        f"the words to your level and encourage you to learn the language."
    )

    exists = await queries.check_user_id(user_id)
    if not exists:
        word = choice(word_selector.all_words())
...
async def get_new_word(message: types.Message):
    user_id = message.from_user.id
    new_word = await word_selector.get_smart_word(user_id)
...
async def user_profile(message: types.Message):
    ...
    if is_premium == 1:
        end_date = await queries.select_premium_end_date(user_id)
...
async def handle_df(message: types.Message, state: FSMContext):
    try:
        ...
        if result:
            await message.answer(formatting.format_definition(result))
        else:
...
async def show_all_words(message: types.Message):
    ...
    await message.answer(
        f"Your words:\n\n{text}\n\n"
        f"To delete a word, use /del_word 'word'\n"
        f"To add a word, use /add_word 'word'"
    )
